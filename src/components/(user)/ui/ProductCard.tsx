  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden bg-gray-50">
        ...
        {/* Quick View — sembunyikan di mobile agar tidak gangguin tap */}
        <div className="hidden sm:flex absolute inset-0 bg-[#1E2753]/0 group-hover:bg-[#1E2753]/5 transition-colors duration-300 items-end justify-center pb-3 opacity-0 group-hover:opacity-100">
          <Link
        ...
        {discountPct && discountPct > 0 && (
          <div className="absolute top-1.5 left-1.5 z-10">
            <span className="flex items-center gap-0.5 bg-red-500 text-white text-[9px] sm:text-[10px] font-black px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg shadow-sm">
              <FontAwesomeIcon icon={faTag} className="w-2 h-2 sm:w-2.5 sm:h-2.5" />-{discountPct}%
            </span>
          </div>
        )}
        ...
        {isBekas && (
          <div className="absolute top-1.5 left-1.5 z-10" style={{ top: discountPct ? "2rem" : undefined }}>
            <span className="bg-amber-500 text-white text-[9px] sm:text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-lg shadow-sm">Second</span>
          </div>
        )}
        ...
        {isOutOfStock && (
          <div className="absolute inset-0 bg-white/80 backdrop-blur-[1px] flex items-center justify-center z-10">
            <div className="bg-gray-800 text-white text-[10px] sm:text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">Stok Habis</div>
          </div>
        )}
      ...
      <div className="p-2.5 sm:p-3 flex flex-col flex-1">
        {/* Kategori */}
        <p className="text-[9px] sm:text-[10px] font-semibold text-[#1E2753]/50 uppercase tracking-widest mb-0.5 sm:mb-1">{product.category}</p>

        {/* Nama */}
        <h3 className="text-[11px] sm:text-xs font-semibold text-gray-800 line-clamp-2 leading-snug flex-1 mb-1.5 sm:mb-2">{product.name}</h3>

        {/* Rating — sembunyikan di layar sangat kecil jika tidak ada */}
        {(product.averageRating ?? 0) > 0 && (
          <div className="flex items-center gap-0.5 sm:gap-1 mb-1 sm:mb-1.5">
            <div className="flex items-center">
              {[1, 2, 3, 4, 5].map((i) => (
                <FontAwesomeIcon key={i} icon={faStar} className={`w-2 h-2 sm:w-2.5 sm:h-2.5 ${i <= Math.round(product.averageRating ?? 0) ? "text-amber-400" : "text-gray-200"}`} />
              ))}
            </div>
            <span className="text-[9px] sm:text-[10px] text-gray-500 font-medium">{product.averageRating?.toFixed(1)}</span>
            {(product.totalReviews ?? 0) > 0 && <span className="hidden sm:inline text-[10px] text-gray-400">({product.totalReviews})</span>}
          </div>
        )}

        {/* Harga */}
        <div className="mb-1">
          {product.originalPrice && product.originalPrice > product.price && <p className="text-[9px] sm:text-[10px] text-gray-400 line-through leading-none mb-0.5">{formatPrice(product.originalPrice)}</p>}
          <p className="text-xs sm:text-sm font-black text-[#1E2753] leading-none">{formatPrice(product.price)}</p>
        </div>
        ...
        <button
          ...
          className={`mt-2 sm:mt-3 w-full py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold rounded-xl transition-all duration-200 flex items-center justify-center gap-1 sm:gap-1.5 ${
        ...
        >
          {adding ? (
            <FontAwesomeIcon icon={faSpinner} className="w-2.5 h-2.5 sm:w-3 sm:h-3 animate-spin" />
          ) : added ? (
            <>
