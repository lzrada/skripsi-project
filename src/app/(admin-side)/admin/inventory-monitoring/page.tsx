...
import {
  updateReorderPointService,
  updateStockParamsService,
  markAsRestockedService,
...
function StockBadge({ stock, rop }: { stock: number; rop: number }) {
  ...
  return (
    <span
      className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${map[level]}`}
    >
      {level}
    </span>
  );
...
function StockBar({ stock, rop }: { stock: number; rop: number }) {
  ...
  const color = isCriticalStock(stock)
    ? "bg-red-500"
    : isLowStock(stock, rop)
    ? "bg-amber-400"
    : "bg-green-500";
  return (
        ...
        {mode === "auto" ? (
          <div className="space-y-4">
            <p className="text-xs text-slate-500 bg-blue-50 border border-blue-100 rounded-lg p-3">
              <span className="font-semibold text-blue-700">Rumus ROP:</span>{" "}
              Rata-rata Penjualan Harian × Lead Time
            </p>
            <div>
              <label className="text-xs font-semibold text-slate-600 mb-1 block">
                Rata-rata Penjualan Harian (unit)
              </label>
              <input
                type="number"
                min={0}
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600 mb-1 block">
                Lead Time (hari pengiriman dari supplier)
              </label>
              <input
                type="number"
                min={1}
          ...
          <div>
            <label className="text-xs font-semibold text-slate-600 mb-1 block">
              Nilai Reorder Point (unit)
            </label>
            <input
              type="number"
              min={0}
              value={manualROP}
              onChange={(e) => setManualROP(Number(e.target.value))}
              className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
            <p className="text-xs text-slate-400 mt-2">
              Sistem akan memberi notifikasi saat stok ≤ {manualROP} unit.
            </p>
          </div>
        )}
...
export default function InventoryMonitoringPage() {
  ...
  const [filter, setFilter] = useState<"semua" | "kritis" | "restock" | "aman">(
    "semua"
  );
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const data: ProductStock[] = snap.docs.map((d) => {
        ...
        const rop: number =
          item.reorderPoint ??
          (avgSales > 0 ? calculateReorderPoint(avgSales, leadTime) : 5);
        return {
          id: d.id,
          name: item.name ?? "",
  ...
  const stats = useMemo(() => {
    ...
    const restock = products.filter(
      (p) => !isCriticalStock(p.stock) && isLowStock(p.stock, p.reorderPoint)
    ).length;
    const aman = products.filter((p) => !isLowStock(p.stock, p.reorderPoint))
      .length;
  ...
  const filtered = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchFilter =
        filter === "semua" ||
        (filter === "kritis" && level === "Stok Kritis") ||
        (filter === "restock" && level === "Perlu Restock") ||
        (filter === "aman" && level === "Stok Aman");
    ...
    <div className="min-h-screen bg-slate-50 p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Pantau ketersediaan barang & titik pemesanan ulang (Reorder Point)
          secara real-time
        </p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 mb-6 flex gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-blue-600 mt-0.5">
            Sistem akan memberi peringatan otomatis saat stok suatu produk
            menyentuh atau di bawah nilai ROP.{" "}
            <span className="font-semibold">
              Rumus: ROP = Rata-rata Penjualan Harian × Lead Time
            </span>
            . Klik tombol <strong>Atur ROP</strong> pada tiap produk untuk
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="text-center px-4 py-3.5 font-semibold text-slate-600 text-xs uppercase tracking-wide">
                  Stok
                </th>
                <th className="text-center px-4 py-3.5 font-semibold text-slate-600 text-xs uppercase tracking-wide">
                  ROP
                ...
                filtered.map((p) => (
                  <tr
                    ...
                    className={`hover:bg-slate-50 transition ${
                      isCriticalStock(p.stock) ? "bg-red-50/50" : ""
                    }`}
                  >
                    ...
                    <td className="px-4 py-4 text-center text-slate-500">
                      {p.averageDailySales > 0 ? p.averageDailySales : "-"}
                    </td>
                    <td className="px-4 py-4 text-center text-slate-500">
                      {p.leadTimeDays} hari
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          ...
                          className="text-xs font-semibold bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 px-3 py-1.5 rounded-lg transition"
                        >
                          Atur ROP
                        </button>
                        {isLowStock(p.stock, p.reorderPoint) && (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-3 mb-3 text-center">
                  ...
                  <div className="bg-slate-50 rounded-lg p-2">
                    ...
                    <p className="font-black text-base text-amber-600">
                      {p.reorderPoint}
                    </p>
                  </div>
                  <div className="bg-slate-50 rounded-lg p-2">
                ...
                <div className="flex gap-2 mt-3">
                  <button
                    ...
                    className="flex-1 text-xs font-semibold bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 py-2 rounded-lg transition"
                  >
                    Atur ROP
                  </button>
                  {isLowStock(p.stock, p.reorderPoint) && (
...
function StatCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`bg-white rounded-2xl border p-4 text-left transition shadow-sm hover:shadow-md ${
        colorMap[color]
      } ${active ? activeRing[color] : "border-slate-200"}`}
    >
