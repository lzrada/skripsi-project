    ...
    <div className="fixed inset-0 z-200 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" onClick={onClose}>
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 mb-5 text-xs text-amber-700">
          ...
          unit — tambahkan stok di atas angka ini agar notifikasi hilang.
        </div>
...
const LowStockAlert = () => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToLowStockProductsService(REORDER_POINT, (data) => setProducts(data));
    return () => unsubscribe();
  }, []);
    ...
    <>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        ...
        <p className="text-xs text-slate-500 mb-3 leading-relaxed">
          Produk di bawah ini telah menyentuh atau melewati batas <span className="font-semibold text-slate-700">Reorder Point (ROP)</span>. Klik <span className="font-semibold">Restock</span> untuk menambah stok.
        </p>
        ...
        {/* Link ke halaman monitoring lengkap */}
