...
const NAV_ITEMS = [
  { href: "/admin/dashboard-admin", label: "Dashboard", icon: <IoHome className="text-xl shrink-0" /> },
  { href: "/admin/product-management", label: "Manajemen Produk", icon: <IoMdPricetag className="text-xl shrink-0" /> },
  { href: "/admin/orders-management", label: "Manajemen Pesanan", icon: <FiList className="text-xl shrink-0" /> },
  { href: "/admin/inventory-monitoring", label: "Monitoring Stok", icon: <MdInventory className="text-xl shrink-0" /> },
  { href: "/admin/coupon-management", label: "Manajemen Kupon", icon: <RiCoupon3Line className="text-xl shrink-0" /> },
];
    ...
    <div className="w-16 sm:w-60 bg-[#1a2035] min-h-screen flex flex-col p-3 sm:p-5 shrink-0 shadow-xl">
      <div className="my-4 hidden sm:flex md:flex items-center gap-2.5">
        <div className="rounded-xl overflow-hidden shadow-md ring-2 ring-white/10 flex-shrink-0">
          <Image src="/images/logo-toko.jpeg" alt="Rizqi Elektronik" width={40} height={40} className="object-cover w-10 h-10" />
        </div>
        <div className="leading-none">
          <p className="text-white font-black text-sm tracking-tight">Rizqi Elektronik</p>
      ...
      >
        <FontAwesomeIcon icon={faRightFromBracket} className={`text-lg shrink-0 w-5 h-5 ${loggingOut ? "animate-pulse" : ""}`} />
